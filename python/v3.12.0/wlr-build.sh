...
if [[ "${WLR_BUILD_FLAVOR}" == *"wasmedge"* ]]; then
  if [[ ! -v WABT_ROOT ]]; then
...
python3 -m pip install --target "${WLR_SOURCE_PATH}/Lib" requests python-dateutil dateparser regex || exit 1
...
if [[ "${WLR_BUILD_FLAVOR}" != *"aio"* && "${WLR_BUILD_FLAVOR}" != *"wasmedge"* ]]; then
